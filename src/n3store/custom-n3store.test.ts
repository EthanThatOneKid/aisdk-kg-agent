...
import { CountInterceptor } from "src/n3store/interceptors/count-interceptor.ts";
import { ErrorInterceptor } from "src/n3store/interceptors/error-interceptor.ts";
...
Deno.test("CustomN3Store - Complete logic path coverage", async (t) => {
  ...
  await t.step("Symbol.iterator with multiple quads", () => {
    ...
    assert(quadIds.includes("http://example.org/test1"));
    assert(quadIds.includes("http://example.org/test2"));
  });
