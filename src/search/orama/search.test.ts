...
import type { SearchHit } from "src/search/search.ts";
import type { OramaTriple } from "src/orama/triple-store.ts";
import {
...
Deno.test("OramaSearchService - search with multiple matches", async () => {
  ...
  const subjects = result.hits.map((hit: SearchHit) => hit.subject);
  ...
  result.hits.forEach((hit: SearchHit) => {
    assert(hit.score > 0);
  });
});
